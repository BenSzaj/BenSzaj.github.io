...
In conclusion, I have presented a preliminary analysis of the sensitivity of the `Price-to-Book` with respect to 62 variables describing additional characteristics of mutual funds and macroeconomic state variables. The analysis suggests that the `Return on Equity` is the single most important feature in my data set. Upon applying a probabilistic neural network to the data, mapping RoE to `Price-to-Book`, we have trained a model which partially describes the data. The model is less than perfect, and leaves significant room for improvement. Nevertheless, the model can be applied to securities at today's prices to estiamte (i) what fair value should be and (ii) how far away from fair value the prices are today given the return on equity. The model suggests that given today's Return on Equity Large Growth stocks are less expensive relative to book value 97.5% of the time. `Small Value` stocks are close to evenly priced givevn their `Return on Equity`, however. This discrepancy might provide guidance towards opportunistic rebalancing. 
